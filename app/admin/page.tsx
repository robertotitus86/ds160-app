...
const styles = {
  ...
  btnGhost: {
    background: '#334155', color: '#fff', border: 'none', borderRadius: 10,
    padding: '9px 12px', cursor: 'pointer'
  } as React.CSSProperties,
  badge: (s: OrderStatus) => ({
  ...
  th: { textAlign: 'left', padding: '8px 10px', borderBottom: '1px solid #1f2937', color: '#cbd5e1' },
  td: { padding: '8px 10px', borderBottom: '1px solid #0b1220' },
};
...
export default function AdminPage() {
  ...
  async function saveToken() {
    if (!hasToken) { alert('Ingresa un token'); return; }
  ...
  async function findById() {
    try {
      if (!searchId.trim()) { alert('Ingresa un ID'); return; }
      setLoading(true);
  ...
  async function act(id: string, action: 'approve' | 'reject') {
    try {
      if (!tokenOk) { alert('Configura tu token primero'); return; }
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      <section style={styles.card as any}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          <input
            ...
            style={{ flex: '1 1 320px', minWidth: 260, padding: 10, borderRadius: 8, border: '1px solid #1f2937', background: '#0b1220', color: '#fff' }}
          />
          <button onClick={saveToken} style={styles.btn}>Usar token</button>
          <button onClick={() => { localStorage.removeItem('admin_token'); setToken(''); setTokenOk(false); }} style={styles.btnGhost}>Quitar token</button>
        </div>
      </section>
      ...
      <section style={styles.card as any}>
        ...
        {found && (
          <div style={{ marginTop: 14, padding: 12, border: '1px solid #1f2937', borderRadius: 10, background: '#0b1220' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', gap: 8 }}>
              <div>
                <div><b>ID:</b> {found.id}</div>
                <div><b>Estado:</b> <span style={styles.badge(found.status)}>{found.status}</span></div>
                {found.createdAt && <div><b>Creada:</b> {new Date(found.createdAt).toLocaleString()}</div>}
                {found.reviewedAt && <div><b>Revisada:</b> {new Date(found.reviewedAt).toLocaleString()}</div>}
                {found.plans?.length ? <div><b>Planes:</b> {found.plans.join(', ')}</div> : null}
                {found.receiptUrl && (
                  <div style={{ marginTop: 6 }}>
                    <b>Comprobante:</b> <button onClick={() => window.open(found.receiptUrl!, '_blank')} style={styles.btnGhost}>Ver</button>
                    <button onClick={() => copy(found.receiptUrl)} style={{ ...styles.btnGhost, marginLeft: 8 }}>Copiar URL</button>
                  </div>
                )}
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => act(found.id, 'approve')} style={styles.btn}>Aprobar</button>
                <button onClick={() => act(found.id, 'reject')} style={styles.btnGhost}>Rechazar</button>
              </div>
            </div>
          </div>
        )}
      ...
      <section style={styles.card as any}>
        ...
        {!tokenOk ? (
          <p style={{ opacity: .8 }}>Ingresa tu token para ver el listado.</p>
        ) : (
            ...
            <table style={styles.table}>
              ...
              <tbody>
                {list.map((o) => (
                  <tr key={o.id}>
                    <td style={styles.td}>
                      <a href="#" onClick={(e)=>{e.preventDefault(); setSearchId(o.id); setFound(o);}} style={{ color:'#93c5fd' }}>
                        {o.id}
                      </a>
                    </td>
                    ...
                    <td style={styles.td}>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        <button onClick={() => act(o.id, 'approve')} style={styles.btn}>Aprobar</button>
                        <button onClick={() => act(o.id, 'reject')} style={styles.btnGhost}>Rechazar</button>
                        {o.receiptUrl && (
                ...
                {list.length === 0 && (
                  <tr>
                    <td colSpan={6} style={{ ...styles.td, opacity: .7 }}>Sin resultados.</td>
                  </tr>
                )}
              </tbody>
            </table>
