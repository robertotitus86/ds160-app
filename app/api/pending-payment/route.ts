...
export async function POST(req: Request) {
  try {
    ...
    lines.push(
      `<p><b>Servicios:</b> ${body.items && body.items.length ? body.items.join(', ') : '(sin items)'}
      </p>`
    );
    lines.push('<hr/>');
    ...
    if (body.method === 'deuna') {
      ...
      lines.push(
        `<p><b>Comprobante:</b> ${body.deuna_file_name || '(no adjunto, revisar referencia)'}</p>`
      );
    }
    if (body.method === 'transferencia') {
      ...
      lines.push(
        `<p><b>Comprobante:</b> ${body.trans_file_name || '(no adjunto, revisar referencia)'}</p>`
      );
    }
