...
// Crea el link de pago con PayPhone. Soporta PROXY_BASE_URL (opcional).
...
async function createPayphoneLinkDirect({ amountUSD, responseUrl, cancelUrl }) {
  ...
  if (!CLIENT_ID || !CLIENT_SECRET || !STORE_ID) {
    return {
      ok: false,
      status: 500,
      message: "Faltan PAYPHONE_CLIENT_ID / PAYPHONE_CLIENT_SECRET / PAYPHONE_STORE_ID"
    };
  }

  // 1) TOKEN (Basic + x-www-form-urlencoded)
...
export async function POST(req) {
  try {
    ...
    // Construye URLs por si no te las mandan desde el frontend
    ...
    // Si tienes un proxy (PROXY_BASE_URL), usarlo:
    const PROXY = process.env.PROXY_BASE_URL; // p.ej. https://mi-proxy.onrender.com
    if (PROXY) {
      ...
      const url =
        data?.prep?.paymentUrl || data?.prep?.url || data?.prep?.payWithCardUrl;
    ...
    // Camino directo a PayPhone (sin proxy)
